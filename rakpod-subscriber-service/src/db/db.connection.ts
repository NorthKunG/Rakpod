  ...
  this.db.getConnection((err: any, connection: any) => {
    if (err) {
      ...
      if (retryCount < maxRetry) {
        console.log(`🔁 Retrying DB connection in ${retryDelay / 1000}s... (attempt ${retryCount + 1}/${maxRetry})`);
        setTimeout(() => this.checkConnection(retryCount + 1), retryDelay);
      } else {
